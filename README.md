...
[![CRAN checks](https://badges.cranchecks.info/worst/wbCorr.svg)](https://CRAN.R-project.org/web/checks/check_results_wbCorr.html)
...
Calling `get_table()` on a `wbCorr` object returns two tables: one for within-cluster correlations and one for between-cluster correlations. See `?get_table` for more information and arguments.
